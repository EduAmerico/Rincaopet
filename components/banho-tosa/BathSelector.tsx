      ...
      {bathTypes.map((bath) => (
        <button key={bath.id} type="button" onClick={() => onSelect(bath.id)}>
          <Card selected={selectedBathId === bath.id} className="cursor-pointer text-left">
            <h4 className="font-semibold text-ink">{bath.name}</h4>
            <p className="mt-1 text-sm text-muted">{bath.description}</p>
            <p className="mt-3 text-sm font-medium text-muted">{bath.durationMin} min</p>
